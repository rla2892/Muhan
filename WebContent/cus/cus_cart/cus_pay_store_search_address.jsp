...
var myaddress = opener.document.getElementById("cus_address").value;
var geocoder = new daum.maps.services.Geocoder();
var addressToCoord = function(result, status) {
    if (status === daum.maps.services.Status.OK) {
        console.log(result[0].x);
        console.log(result[0].y);
        var mylocation = new daum.maps.LatLng(result[0].y,result[0].x)
	    ...
	    '  <a href="cus_customer_info.do" target="_blank">' +
	    '    <span class="title">'+"주문 위치"+'</span>' +
	    '  </a>' +
	    '</div>';
        ...
        var positions=[];
    	for(var i=0; i<store_id_values.length;i++){
    		positions.push({
...
geocoder.addressSearch(myaddress,addressToCoord);
